...
export default function ChatListUpdate ( {usernameinlogin, chats, createScreen, connection} ) {
    ...
    const chatList = chats.map((friend, key) => {
        return <SideBarChat usernameinlogin={usernameinlogin} username={friend.id} nickname={friend.name} image={friend.image} server={friend.server} createScreen={createScreen} key={key} connection={connection}/>;
    });
